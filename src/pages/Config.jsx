    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md">
        ...
        <div className="space-y-4">
          <div className="space-y-2"><label>Latitude</label><input name="lat" value={formState.lat} onChange={handleChange} className="w-full bg-gray-700 p-2 rounded" /></div>
          <div className="space-y-2"><label>Longitude</label><input name="lon" value={formState.lon} onChange={handleChange} className="w-full bg-gray-700 p-2 rounded" /></div>
          <div className="space-y-2"><label>Tolerância (metros)</label><input name="tolerance" type="number" value={formState.tolerance} onChange={handleChange} className="w-full bg-gray-700 p-2 rounded" /></div>
          <div className="space-y-2"><label>Hora de Entrada</label><input name="clockInHour" type="number" value={formState.clockInHour} onChange={handleChange} className="w-full bg-gray-700 p-2 rounded" /></div>
          <div className="space-y-2"><label>Hora de Saída</label><input name="clockOutHour" type="number" value={formState.clockOutHour} onChange={handleChange} className="w-full bg-gray-700 p-2 rounded" /></div>
          <button onClick={handleSave} className="w-full bg-green-600 hover:bg-green-700 p-3 rounded flex items-center justify-center space-x-2">
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={() => handleOpenModal()} className="bg-blue-600 hover:bg-blue-700 p-2 rounded flex items-center space-x-2">
      ...
      <div className="space-y-3">
        {configs.map(config => (
          <div key={config.id} className="bg-gray-800 p-4 rounded-lg flex justify-between items-center">
            <div>
              <p className="font-semibold">ID: {config.id}</p>
        ...
        {configs.length === 0 && <p className="text-gray-500 text-center py-4">Nenhuma configuração encontrada.</p>}
      </div>
